...
describe("safeSessionStorage", () => {
  describe("writeJson", () => {
    it("stores a value and retrieves it with readJson", () => {
      ...
      const result = readJson("test_key");
      expect(result).toEqual({ hello: "world" });
    });

    it("returns true on successful write", () => {
      const success = writeJson("test_key", { data: 123 });
    ...
    it("handles null values", () => {
      ...
      // readJson returns fallback for null parses
  ...
  describe("readJson", () => {
    ...
    it("returns fallback when storage throws", () => {
      // This is a read-only test; storage is available in jsdom
      // We just verify the function doesn't throw
      expect(() => readJson("anything")).not.toThrow();
    });
  });
  ...
  describe("user object round-trip (realistic scenario)", () => {
    ...
    it("stores and retrieves authority state as AuthContext does", () => {
      const authority = {
        authorityVersion: 3,
        permissions: ["manage_equipment", "view_reports"],
        role: "hospital",
        active: true,
      };
