...
function StatusPill({ status, map }) {
  const meta = (map || CRYO_STATUS)[status] || { label: status, cls: "text-slate-400 bg-slate-500/10 border-slate-500/30" };
...
function MiniSparkline({ points, tone = "sky", width = 130, height = 38, min = null, max = null }) {
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible" aria-label="telemetry sparkline">
  ...
  return (
    <div className="relative w-full sm:w-72">
      ...
      <input
        ...
        className="w-full rounded-xl border border-slate-800 bg-slate-950/80 py-2.5 pl-10 pr-4 text-sm text-slate-200 placeholder-slate-600 outline-none transition focus:border-cyan-500/50 focus:ring-2 focus:ring-cyan-500/20"
      />
    </div>
  );
}
      ...
      <div className={`relative w-full ${wide ? "max-w-3xl" : "max-w-xl"} max-h-[86vh] overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/60 animate-scale-up`}>
        <div className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-slate-800 bg-slate-900/95 px-5 py-4 backdrop-blur">
          <div className="flex items-center gap-3">
            <div className="rounded-xl border border-slate-700 bg-slate-800 p-2 text-cyan-400">
              <Icon size={18} />
            </div>
            <div>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              disabled={exporting}
              className="flex items-center gap-2 rounded-xl border border-cyan-500/40 bg-cyan-500/10 px-3.5 py-2.5 text-xs font-bold text-cyan-400 transition hover:bg-cyan-500/20 disabled:opacity-60"
            >
