...
const seededSeries = (seed, n = SEED_POINTS, base = 90, amp = 12) => {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    v = clamp(v + r * amp + (base - v) * 0.08, base - amp, base + amp);
    pts.push(round1(v));
  }
  return pts;
};
...
function MiniSparkline({ points, tone = "sky", width = 130, height = 38 }) {
  ...
  const stroke = { sky: "#38bdf8", rose: "#fb7185", amber: "#fbbf24", emerald: "#34d399", violet: "#a78bfa" }[tone] || "#38bdf8";
              ...
              <div>
                ...
                <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-400">
                  <span className="flex items-center gap-1.5">
                    ...
                    {playing ? `Streaming · tick #${tick}` : "Stream paused"}
                  </span>
